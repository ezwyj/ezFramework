...
namespace ValueSet.Service
{
    public class ValueSetService
    {
        ...
        public ValueSet.Entity.ValueSet GetValueSetByName(string name)
        {
            var vs = db.Fetch<ValueSet.Entity.ValueSet>(" where text=@0 and isdelete = 0", name).FirstOrDefault();
            return vs;

        }
        public ValueSet.Entity.ValueSet GetValueSetByID(int id)
        {
            var vs = db.Fetch<ValueSet.Entity.ValueSet>(" where id=@0", id).Where(o => o.IsDelete == false).FirstOrDefault();
            return vs;

        }
        public ValueSet.Entity.ValueItem GetValueItemByID(int id)
        {
        ...
        public List<string> ValidateItem(ValueItem rec)
        {
            ...
            {
                var vs = db.Fetch<ValueSet.Entity.ValueSet>("where isdelete=0 and id=@0", rec.SetId);
                if (vs == null)
                {
                    errors.Add("值集ID不正确。");
                }
        ...
        {
            string msg = "";
            var it = db.Single<ValueSet.Entity.ValueSet>(id);

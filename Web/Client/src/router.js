	...
	routes: [
		{
			...
			beforeEnter(to, from, next) {
				if (process.env.NODE_ENV === 'production') {
					location.href = OP_CONFIG.loginUrl + '?returnUrl=' + encodeURIComponent(to.query.returnUrl);
					return;
				}
				next();
			},
		...
		},
		{
			...
			async beforeEnter(to) {
				if (process.env.NODE_ENV === 'production') {
					location.href = OP_CONFIG.logoutUrl + '?returnUrl=' + encodeURIComponent(to.query.returnUrl);
					return;
				}
